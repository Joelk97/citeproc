export const localeEnUS = `<?xml version="1.0" encoding="utf-8"?>
...
export function normalizeCslItem(item) {
  const out = { ...item };

  for (const key of CSL_NAME_VARS) {
  ...
  if (!out.type) out.type = "article";
  if (!out.issued || !out.issued["date-parts"]) {
    out.issued = { "date-parts": [[0]] };
  }

  return out;
}

export const CSL_NAME_VARS = [
  "author",
  "editor",
  "translator",
  "container-author",
